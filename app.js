// app.js — Uragráfica (versión segura con logs y edición)
import {
...
/* ====== Helper de logs ====== */
function log(...args){ try{ console.log("[Uragrafica]", ...args);}catch{} }
...
log("DOM listo:", {board:!!board, inOrden:!!inOrden, inCliente:!!inCliente, inProducto:!!inProducto, inEstado:!!inEstado, btnAdd:!!btnAdd});

if (!board) {
  alert("No encuentro el contenedor principal (#board). Revisa tu index.html");
}

/* ====== Poblamos el select de estado si existe ====== */
if (inEstado) {
  ESTADOS.forEach(e => {
...
try {
    ...
    tmp.forEach(async o => {
      if (o.estado === "Despachado") {
        try {
          ...
          o.estado = "Realizado";
        } catch(e){ console.warn("No pude migrar un doc:", o.id, e.message); }
      }
    });

    ORDERS = tmp;
    log("Docs recibidos:", ORDERS.length);
    render();
  ...
  function renderCard(o){
    ...
    btnDel.onclick   = ()   => confirm("¿Eliminar esta orden?") && remove(o);
  ...
  btnAdd?.addEventListener("click", async () => {
    if (!inCliente || !inProducto || !inEstado) return alert("Faltan campos del formulario en el HTML.");
  ...
  // Si el input de orden existe y está vacío, sugiere 001
  if (inOrden && !inOrden.value) inOrden.value = "001";

} catch (e) {
  console.error("Error montando la app:", e);
  alert("Error inicializando la app. Revisa la consola del navegador.");
}
